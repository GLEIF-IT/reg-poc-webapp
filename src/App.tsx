      ...
      <Drawer open={drawerOpen} onClose={toggleDrawer(false)}>
        ...
        {client !== null && status === 'Connected' &&
          <div style={{ marginTop: 'auto', textAlign: 'center' }}>
            <Divider />
            <List>
              {[reduceString(selectedOption1), reduceString(selectedOption2)].map((text, index) => (
                <Tooltip
                  title={index == 0 ? 'Identifier' : 'Credential'}
                  placement="right"
                ...
                <StepContent>
                  ...
                  {index === 1 && (
                    <>
                      ...
                      >
                        {aids.map((aid, index) => (
                          <FormControlLabel key={index} value={aid.prefix} control={<Radio />} label={aid.prefix} />
                        ))}
                      </RadioGroup>
                      <Button
                        variant="contained"
                    ...
                    <>
                      ...
                      >
                        {acdcs.map((acdc, index) => (
                          <FormControlLabel key={index} value={acdc} control={<Radio />} label={acdc} />
                        ))}
